...
On macOS, `--passwords-app` takes precedence over `SECRET_BACKEND` when both are given: it is a deliberate, per-invocation flag, whereas the environment variable is typically inherited from a shell profile or CI environment set once for a whole session. When the flag is given, `SECRET_BACKEND` does not affect the outcome — an unrelated value it happens to hold cannot break the invocation. It is, however, still checked: a value that would otherwise be a hard error here is reported as a warning on stderr, so a broken `SECRET_BACKEND` inherited alongside a `--passwords-app` alias doesn't go unnoticed until `git-credential-secret` — which cannot take the flag at all — hits the same value cold and reports it as a hard error instead.
