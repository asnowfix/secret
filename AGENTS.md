...
3. Wire it into the appropriate `cmd/backend_<os>.go` file's name-to-backend switch (`resolveBackendName` on darwin, `selectBackend()` directly on linux/windows, which have no flag-validation path to share it with), giving it a `SECRET_BACKEND` name (add it to that file's `<os>BackendNames` slice and to `knownBackendNames` in `cmd/backend_common.go` so unrecognised-value errors on other platforms can name it correctly). The `<os>BackendNames` slice is checked against that switch's own cases by that platform's `TestSelectBackend_<OS>_AllOwnNamesAccepted` test (`cmd/backend_<os>_test.go`) — both live in the same build, so if you add a name to one and forget the other, that test fails. On darwin, `TestSelectBackend_Darwin_FlagPathSilentForAllOwnNames` covers the `--passwords-app` validation path too, because it shares `resolveBackendName` with the normal path rather than carrying its own copy of the name list — do not reintroduce a second switch or list there. `knownBackendNames` cannot be checked this way: the build-tag architecture means no single build can see another platform's switch, so keeping that one in sync across platforms is still on you (see its doc comment in `cmd/backend_common.go`).
